...
export class ConfigurarReportesComponent implements OnInit, OnDestroy {
  ...
  listarOpciones() {
    ...
    this.resumenGeneralApiService
      .listarEleccionesParaConfigurarReporte(data)
      .pipe(take(1))
      .subscribe({
        next: (result) => {
          if (result.success) {
    ...
    this.dialogService.mostrarMensajeConfirmacion(`¿Está seguro de realizar la operación?`).subscribe({
      next: (result) => {
        if (result) {
          ...
          const fechaDeInicioDeGeneracion = this.fechaDeInicioDeGeneracionCtrl.value;

          if (!fechaDeInicioDeGeneracion) {
            throw new Error('fechaDeInicioDeGeneracion requerida');
          }

          const params = {
            eleccion: currentElection?.text,
            eleccionId: currentElection?.value,
            fechaInicio: this.getStringDateFromNumberDate(fechaDeInicioDeGeneracion),
  ...
  private disableFechaDeInicio() {
    this.fechaDeInicioDeGeneracionCtrl.disable();
  }

  private getNumberDateFromStringDate(date: string): Date {
