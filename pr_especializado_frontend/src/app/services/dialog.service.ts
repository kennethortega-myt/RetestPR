...
import { DEFAULT_DIALOG_SIZE, DIALOG_CONFIGS, DialogKey, IDialogConfigData, TYPE_ICON_MAP } from '../components/dialog/dialog.constants';
import { DialogComponent } from '../components/dialog/dialog.component';
import { PopupProximanenteComponent } from '../components/popup-proximanente/popup-proximanente.component';
...
export class DialogService {
    ...
    private _openDialog(dialogDataConfig: IDialogConfigData, overrideConfig?: MatDialogConfig & { useRootContainer?: boolean }): Observable<boolean> {
        ...
        if (shouldPreloadImage) {
            ...
            if (dialogDataConfig.iconPath) {
                ...
                dialogDataConfig.key in DIALOG_CONFIGS
            ) {
    ...
    verDialogPersonalizado(keyOrData: string | IDialogConfigData, message?: string, title?: string, config?: MatDialogConfig): void {
        this.mostrarMensaje(keyOrData, message, title, config);
    }
    ...
    cerrarTodos(): void {
        for (const ref of this.dialogRefs) {
            ref.close();
        }
        this.dialogRefs = [];
    }
    ...
    mostrarPopupProximamente(): void {
        const isMobile = window.innerWidth <= 960;
        this.openComponent(
            PopupProximanenteComponent,
            {
                width: isMobile ? '90vw' : 'auto',
                minWidth: 'auto',
                maxWidth: isMobile ? 'min(96vw, 400px)' : '860px',
                panelClass: 'popup-proximamente-panel'
            }
        );
    }
}
