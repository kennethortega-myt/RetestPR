...
import { DEFAULT_DIALOG_SIZE, DIALOG_CONFIGS, DialogKey, IDialogConfigData, TYPE_ICON_MAP } from '../components/dialog/dialog/dialog.constants';
...
export class DialogService {
    ...
    show(keyOrData: string | IDialogConfigData, message?: string, title?: string, config?: MatDialogConfig): Observable<boolean> {
        ...
        // Si hay una configuración predefinida para la key, mezclarla aquí
        if (data.key && Object.prototype.hasOwnProperty.call(DIALOG_CONFIGS, data.key)) {
            const predefined = DIALOG_CONFIGS[data.key as DialogKey];
            // Las propiedades pasadas por el llamador (data) deben sobrescribir a las predefinidas
            data = { ...predefined, ...data } as IDialogConfigData;
        } else {
            console.warn('Initial dialog data before overrides:', data);
        }
