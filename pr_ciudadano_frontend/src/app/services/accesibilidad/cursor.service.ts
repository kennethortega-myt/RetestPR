...
import { AccessibilityStep, Profile } from '../../components/accesibilidad/accesibilidad.interface';
import { encryptStorageLocal } from '../../settings/encrypt-storage.settings';
...
export class CursorService {
  ...
  textSizeActiveUpdate(value: AccessibilityStep): void {
    ...
    encryptStorageLocal.setItem(KEY_STORE_ACCESSIBILITY.textSizeActive, value);
  }
  contrastActiveUpdate(value: AccessibilityStep): void {
    this.contrastActive.set(value);
    encryptStorageLocal.setItem(KEY_STORE_ACCESSIBILITY.contrastActive, value);
  }

  readingMaskActiveUpdate(value: AccessibilityStep): void {
    this.readingMaskActive.set(value);
    encryptStorageLocal.setItem(KEY_STORE_ACCESSIBILITY.readingMaskActive, value);
  }

  dislexiaFriendlyActiveUpdate(value: AccessibilityStep): void {
    this.dislexiaFriendlyActive.set(value);
    encryptStorageLocal.setItem(KEY_STORE_ACCESSIBILITY.dislexiaFriendlyActive, value);
  }

  cursorActiveUpdate(value: AccessibilityStep): void {
    this.cursorActive.set(value);
    encryptStorageLocal.setItem(KEY_STORE_ACCESSIBILITY.cursorActive, value);
  }

  interlineadoActiveUpdate(value: AccessibilityStep): void {
    this.interlineadoActive.set(value);
    encryptStorageLocal.setItem(KEY_STORE_ACCESSIBILITY.interlineActive, value);
  }

  profileActiveUpdate(value: Profile): void {
    this.profileActive.set(value);
    encryptStorageLocal.setItem(KEY_STORE_ACCESSIBILITY.profileActive, value);
  }

  loadDataFromLocalStorage(): void {
    const _textSizeActive = encryptStorageLocal.getItem<string>(KEY_STORE_ACCESSIBILITY.textSizeActive);
    if (_textSizeActive) {
      this.textSizeActive.set(JSON.parse(_textSizeActive));
    }

    const _contrastActive = encryptStorageLocal.getItem<string>(KEY_STORE_ACCESSIBILITY.contrastActive);
    if (_contrastActive) {
      this.contrastActive.set(JSON.parse(_contrastActive));
    }

    const _readingMaskActive = encryptStorageLocal.getItem<string>(KEY_STORE_ACCESSIBILITY.readingMaskActive);
    if (_readingMaskActive) {
      this.readingMaskActive.set(JSON.parse(_readingMaskActive));
    }

    const _dislexiaFriendlyActive = encryptStorageLocal.getItem<string>(KEY_STORE_ACCESSIBILITY.dislexiaFriendlyActive);
    if (_dislexiaFriendlyActive) {
      this.dislexiaFriendlyActive.set(JSON.parse(_dislexiaFriendlyActive));
    }

    const _cursorActive = encryptStorageLocal.getItem<string>(KEY_STORE_ACCESSIBILITY.cursorActive);
    if (_cursorActive) {
      this.cursorActive.set(JSON.parse(_cursorActive));
    }

    const _interlineadoActive = encryptStorageLocal.getItem<string>(KEY_STORE_ACCESSIBILITY.interlineActive);
    if (_interlineadoActive) {
      this.interlineadoActive.set(JSON.parse(_interlineadoActive));
    }

    const _profileActive = encryptStorageLocal.getItem<string>(KEY_STORE_ACCESSIBILITY.profileActive);
    if (_profileActive) {
  ...
  resetAll(): void {
    encryptStorageLocal.removeItem(KEY_STORE_ACCESSIBILITY.textSizeActive);
    encryptStorageLocal.removeItem(KEY_STORE_ACCESSIBILITY.contrastActive);
    encryptStorageLocal.removeItem(KEY_STORE_ACCESSIBILITY.readingMaskActive);
    encryptStorageLocal.removeItem(KEY_STORE_ACCESSIBILITY.dislexiaFriendlyActive);
    encryptStorageLocal.removeItem(KEY_STORE_ACCESSIBILITY.cursorActive);
    encryptStorageLocal.removeItem(KEY_STORE_ACCESSIBILITY.interlineActive);
    encryptStorageLocal.removeItem(KEY_STORE_ACCESSIBILITY.profileActive);
