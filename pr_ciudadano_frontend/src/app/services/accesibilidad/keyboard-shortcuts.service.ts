...
import { KeyboardShortcut, ShortcutConfig, ValidKey, ModifierKey } from '../../interfaces/keyboard-shortcuts.interfaces';
import { encryptStorageLocal } from '../../settings/encrypt-storage.settings';
import { KEY_STORE_ACCESSIBILITY } from '../../components/accesibilidad/accesibilidad.constant';
...
export class KeyboardShortcutsService implements OnDestroy {
  ...
  private configSubject = new BehaviorSubject<ShortcutConfig>(this.loadConfig());
  ...
  registerShortcut(shortcut: KeyboardShortcut): void {
    ...
    if (!this.hasModifierKey(shortcut.keys)) {
      console.warn(
        `[WCAG 2.1.4] El atajo "${shortcut.id}" debe incluir una tecla modificadora (Ctrl, Alt, Meta).`
      );
      return;
    }
  ...
  private hasModifierKey(keys: ValidKey[]): boolean {
    const modifiers: ModifierKey[] = ['Control', 'Alt', 'Meta', 'Shift'];
    return keys.some(key => modifiers.includes(key as ModifierKey));
  }
  ...
  private matchesCrossPlatform(event: KeyboardEvent, keys: ValidKey[]): boolean {
    ...
    if (keys.includes('Meta')) {
      const pcKeys = keys.map(key => key === 'Meta' ? 'Alt' as ValidKey : key);
      const pressedKeys: ValidKey[] = [];
      ...
      if (pcKeys.length === pressedKeys.length && pcKeys.every(key => pressedKeys.includes(key))) {
        return true;
      }
    }
    ...
    if (keys.includes('Alt')) {
      const macKeys = keys.map(key => key === 'Alt' ? 'Meta' as ValidKey : key);
      const pressedKeys: ValidKey[] = [];
      ...
      if (macKeys.length === pressedKeys.length && macKeys.every(key => pressedKeys.includes(key))) {
        return true;
      }
    }
  ...
  private loadConfig(): ShortcutConfig {
    const stored = encryptStorageLocal.getItem<string>(KEY_STORE_ACCESSIBILITY.keyboardShortcuts);
    if (stored) {
      try {
  ...
  private saveConfig(config: ShortcutConfig): void {
    encryptStorageLocal.setItem(KEY_STORE_ACCESSIBILITY.keyboardShortcuts, JSON.stringify(config));
  }
