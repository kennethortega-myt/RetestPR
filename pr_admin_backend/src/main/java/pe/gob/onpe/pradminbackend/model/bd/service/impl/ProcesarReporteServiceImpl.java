...
import pe.gob.onpe.pradminbackend.model.bd.service.ProcesarReporteService;
...
public class ProcesarReporteServiceImpl implements ProcesarReporteService {
    ...
    public void enviarPeticionProcesarReporte(String idReporte) {
        ...
        try {
                ...
                ResponseEntity<ReporteCronResponse> responseService = restTemplate.getForEntity(
                urlServicioProcesarReporte +"/reporteProgramado/procesar/{idReporte}"
                 , ReporteCronResponse.class,idReporte);

            if (responseService.getStatusCode() == HttpStatus.OK) {
                log.info("Peticion enviada, reporte: {} , respuesta: {}  " , idReporte,responseService.getBody().getResponse().getMessage());
